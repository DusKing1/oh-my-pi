...
type SwiftBridgeRelease = unsafe extern "C" fn(usize);
type SwiftErrorRetain = unsafe extern "C" fn(*mut c_void);
...
type ObjcRetain = unsafe extern "C" fn(*mut c_void) -> *mut c_void;
...
impl SnapshotStringBridge {
	fn load(runtime: &Runtime) -> std::result::Result<Self, BridgeFailure> {
		Ok(Self {
			...
			// SAFETY: objc_retain has the declared Objective-C ABI.
			retain:         unsafe { runtime.symbol(c"objc_retain")? },
	...
	fn to_rust(self, value: SwiftString) -> std::result::Result<String, BridgeFailure> {
		...
		let result = if ns_string.is_null() {
			...
			// SAFETY: The bridge returned a live NSString which retain keeps alive while
			// CoreFoundation reads it.
			let ns_string = unsafe { (self.retain)(ns_string) };
			let result = ns_string_to_rust_with(ns_string, self.length, self.maximum, self.copy);
			// SAFETY: objc_retain returned a +1 reference.
		...
		unsafe {
			if value.object != 0 {
				(self.bridge_release)(value.object);
			}
		}
		result
	}
		...
		match receiver.recv_timeout(RECEIVE_INTERVAL) {
			...
			Ok(StreamMessage::Complete(Ok(()))) => {
				return Ok(AppleFmGeneration {
					prompt_tokens_estimated: token_estimate(prompt.as_str()),
...
fn create_session(
	...
	// SAFETY: swift_release has the declared Swift reference-counting ABI.
	let release: SwiftRelease = unsafe { runtime.symbol(c"swift_release")? };
	// SAFETY: The model and optional instructions carry the +1 ownership consumed
	// by the initializer, and metadata is complete.
	let session = unsafe {
...
fn create_stream(
	...
	unsafe {
		apple_fm_stream_response(
			...
			prompt.object,
			options,
			runtime.symbol::<*const c_void>(STREAM_RESPONSE)?,
		);
	};
...
impl Request {
	fn advance(&mut self, error: *mut c_void) -> NextAction {
		...
		match text {
			Ok(text) => {
				if self.sender.send(StreamMessage::Snapshot(text)).is_ok() {
					NextAction::Continue
				} else {
					NextAction::Complete(Err(BridgeFailure::runtime(
...
fn complete_request(request: *mut Request, result: std::result::Result<(), BridgeFailure>) {
	...
	let cleared = APPLE_FM_ACTIVE_REQUEST
		...
		.is_ok();
	if cleared {
		let _ = request_ref.sender.send(StreamMessage::Complete(result));
