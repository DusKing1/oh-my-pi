...
//!
//! `combine FILE1 OP FILE2` accepts the case-insensitive operators `and`,
//! `not`, `or`, and `xor`. `-` names stdin, but only for one operand. Lines
//! remain raw byte strings; membership ignores a trailing newline, while output
//! preserves each original line exactly.
