...
fn rewrite_tail_argv(mut argv: Vec<OsString>) -> Result<Vec<OsString>, String> {
	...
	if clap_ok && !force_obsolete_blocks && !obsolete_token.to_string_lossy().starts_with('+') {
		return Ok(argv);
	}
...
fn uu_tail(settings: &Settings, host: &mut Host) -> TailResult<()> {
	...
	let mut observer =
		Observer::from(settings, host.stdout_clone(), host.stderr_clone(), host.cancel_flag());
