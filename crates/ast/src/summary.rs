...
use crate::{Result, language::SupportLang, parse_cache::parse_cached};

const DEFAULT_MIN_BODY_LINES: u32 = 4;
const DEFAULT_MIN_COMMENT_LINES: u32 = 6;

/// Owned inputs and thresholds for [`summarize_code`].
...
/// Borrowed inputs and thresholds for [`summarize_source`].
#[derive(Debug, Clone, Copy, Default)]
pub struct SummarySettings<'a> {
	/// Language alias used before path inference.
	pub lang:               Option<&'a str>,
	/// File path used to infer language when `lang` is omitted.
	pub path:               Option<&'a str>,
	/// Minimum total node lines before eliding a body or literal.
	pub min_body_lines:     Option<u32>,
	/// Minimum comment lines before eliding a multiline block comment.
	pub min_comment_lines:  Option<u32>,
	/// Target visible-line count for breadth-first unfolding.
	pub unfold_until_lines: Option<u32>,
	/// Hard visible-line ceiling for breadth-first unfolding.
	pub unfold_limit_lines: Option<u32>,
}

/// One kept or elided range in a structural summary.
...
/// Structural summary metadata and ordered source segments.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
...
impl ElidableForest {
	fn push(&mut self, parent: Option<usize>, span: LineSpan) -> usize {
		...
		self
			.nodes
			.push(SpanNode { span, children: SmallVec::new() });
		match parent {
...
fn select_folded_spans(
	...
	let mut folded: SparseSet<usize> = forest.roots.iter().copied().collect();
	...
	let mut queue: SmallVec<usize, 8> = forest.roots.iter().copied().collect();
	let mut queue_index = 0;

	while let Some(&idx) = queue.get(queue_index) {
		queue_index += 1;
...
/// Summarizes owned source code while preserving the original owning API.
pub fn summarize_code(options: SummaryOptions) -> Result<SummaryResult> {
	...
	summarize_source(&code, SummarySettings {
		lang: lang.as_deref(),
		path: path.as_deref(),
		min_body_lines,
		min_comment_lines,
...
/// Summarizes borrowed source code without requiring an owning input copy.
pub fn summarize_source(source: &str, settings: SummarySettings<'_>) -> Result<SummaryResult> {
	let total_lines = count_lines(source);
	if source.is_empty() {
		return Ok(unparsed_result(source, total_lines));
	}

	let Some(language) = resolve_language(settings.lang, settings.path) else {
		return Ok(unparsed_result(source, total_lines));
	};
	...
	let min_body_lines = settings
		.min_body_lines
		.unwrap_or(DEFAULT_MIN_BODY_LINES)
		.max(2);
	let min_comment_lines = settings
		.min_comment_lines
		.unwrap_or(DEFAULT_MIN_COMMENT_LINES)
		.max(4);
	let unfold_until = settings.unfold_until_lines.unwrap_or(0);
	let unfold_limit = settings
		.unfold_limit_lines
...
fn unparsed_result(source: &str, total_lines: u32) -> SummaryResult {
...
fn normalize_spans(mut spans: Vec<LineSpan>, total_lines: u32) -> Vec<LineSpan> {
	...
	for index in 0..spans.len() {
		...
		if merged_len > 0 {
			let last = &mut spans[merged_len - 1];
			if span.start <= last.end.saturating_add(1) {
				last.end = last.end.max(span.end);
				continue;
			}
		}
		spans[merged_len] = span;
		merged_len += 1;
	}
	spans.truncate(merged_len);
	spans
}
...
fn build_segments(source: &str, total_lines: u32, spans: &[LineSpan]) -> Vec<SummarySegment> {
	...
	let mut segments = Vec::with_capacity(spans.len().saturating_mul(2).saturating_add(1));
	let mut span_index = 0;
	let mut current_kind: Option<&str> = None;
	let mut current_start = 1;
	let mut current_text = String::new();
	let mut current_line_count = 0_u32;

	for (line_number, line) in (1..=total_lines).zip(source.lines()) {
		while spans
			.get(span_index)
			.is_some_and(|span| span.end < line_number)
		{
			span_index += 1;
		}
		let is_elided = spans
			.get(span_index)
			.is_some_and(|span| span.start <= line_number);
	...
	if let Some(kind) = current_kind {
		push_segment(&mut segments, kind, current_start, total_lines, current_text);
	}
	segments
}
...
mod tests {
	...
	fn summarizes_fortran_program() {
		...
		assert_ne!(result.segments.len(), 0);
	}
