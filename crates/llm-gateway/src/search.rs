...
pub struct SearchRegistry {
	...
	configured_order: SmallVec<SmolStr, 10>,
}
...
fn unify(spec: &EngineSpec, result: EngineResult, query: &ParsedQuery) -> SearchResponse {
	let (answer, sources, citations) = match result.payload {
		EnginePayload::Raw { sources } => (SmolStr::default(), sources, Vec::new()),
	...
	warnings.extend(filtered.relaxed.into_iter().map(|label| {
		SmolStr::from(format!("no results matched `{label}`; the constraint was relaxed"))
	}));
	SearchResponse::builder()
		.engine(SmolStr::from(spec.id))
		.answer(answer)
...
struct ParsedQuery {
	sites:          Vec<SmolStr>,
	excluded_sites: Vec<SmolStr>,
	in_url:         Vec<SmolStr>,
	in_title:       Vec<SmolStr>,
	filetypes:      Vec<SmolStr>,
	after:          Option<SmolStr>,
	before:         Option<SmolStr>,
	terms:          Vec<QueryTerm>,
}
...
fn normalize_site(value: &str) -> SmolStr {
	let value = value
		.trim()
...
mod tests {
	...
	#[derive(Default)]
	struct Credentials(HashMap<SmolStr, SmolStr>);
	...
	fn source(url: &str, title: &str) -> SearchSource {
		SearchSource::builder()
			...
			.snippet(SmolStr::new_static("snippet"))
			.published_at(SmolStr::new_static("2025-01-02"))
	...
	async fn all_three_paradigms_map_to_one_shape() {
		let citation = SearchCitation::builder()
			.url(SmolStr::new_static("https://example.com"))
			.title(SmolStr::new_static("Example"))
			.cited_text(SmolStr::new_static("evidence"))
			.build();
		let cases = [
