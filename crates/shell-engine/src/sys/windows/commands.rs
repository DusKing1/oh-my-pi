...
pub trait CommandSessionExt {
	...
	/// Like [`CommandSessionExt::detach_session`]. On Windows there is no
	/// session or `fork`-based reparenting, so this is a no-op: the operand
	/// stays a child of the shell.
