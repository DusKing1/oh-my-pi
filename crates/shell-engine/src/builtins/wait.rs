	...
	async fn execute<SE: crate::ShellExtensions>(
		...
		if !self.ids.is_empty() {
			for id in &self.ids {
				if id.starts_with('%') {
					// It's a job spec.
					match context.shell.jobs_mut().resolve_job_spec(id) {
						Ok(job) => {
						...
						Err(error) => {
							writeln!(context.stderr(), "{}: {}: {}", context.command_name, id, error)?;
...
fn resolve_wait_selectors<SE: crate::ShellExtensions>(
	...
	for id in ids {
		if id.starts_with('%') {
			match context.shell.jobs().resolve_job_spec_selector(id) {
