...
fn main() {
	...
	let config = config_path
		...
		.unwrap_or_else(|| {
			panic!(
				"PYO3_CONFIG_FILE (got {:?}) must point at an existing pyo3-config.txt generated by \
				 omp-py's scripts/fetch-python.sh; run crates/py/scripts/fetch-python.sh first",
				config_path
			)
		});
	let vendor = config
